...
process.s.weights <- function(s.weights, data = NULL) {
    ...
    if (is_not_null(s.weights)) {
        if (!(is.character(s.weights) && length(s.weights) == 1) && !is.numeric(s.weights)) {
            stop("The argument to s.weights must be a vector or data frame of sampling weights or the (quoted) name of the variable in data that contains sampling weights.", call. = FALSE)
        }
