...
package for balance assessment. See
...
from the sample (by default, the smallest weight allowed is
\\10^{-8}\\).

We can use [`plot()`](https://rdrr.io/r/graphics/plot.default.html) to
examine the dual variables for each constraint, which represent how
active that constraint is at the optimal point. Highly active
constraints affect the objective function value the most when their
tolerances are changed.
...
which implements the methods described in Jackson, Rhodes, and Ouwens
([2021](#ref-jacksonAlternativeWeightingSchemes2021)) for
